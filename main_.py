...
def on_startup():
    ...
    # 신규 환경이면 seed_latest.json 자동 import (DB 가 비어있을 때만)
    try:
        from import_seed import auto_import_if_empty
        result = auto_import_if_empty(silent=False)
        if result:
            stats = result["stats"]
            print(f"  📦 시드 자동 import 완료 — "
                  f"문제 {stats['essay_problems']['created']}개, "
                  f"그룹 {stats['problem_groups']['created']}개, "
                  f"프롬프트 {stats['system_prompts']['created']}개")
    except Exception as e:
        print(f"  ⚠  시드 자동 import 실패 (무시 가능): {e}")
